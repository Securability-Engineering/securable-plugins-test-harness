...
#
# Automates OpenCode CLI to generate a project from a PRD in 4 languages,
...
#
# Usage:
#   ./run-codegen-opencode.sh --prd <file> [--output-dir <dir>] [--plugin-repo <url>] [--dry-run] [--resume] [--modes <list>] [--languages <list>]
...
if [[ -d "$PLUGIN_TEMP" ]]; then
    write_step "Module already cloned at $PLUGIN_TEMP — updating"
    if [[ "$DRY_RUN" == true ]]; then
        _yellow "  [DRY-RUN] git -C $PLUGIN_TEMP pull --ff-only"
    else
        if [[ ! -d "$PLUGIN_TEMP/.git" ]]; then
            _red "Error: Existing module cache is not a git repository: $PLUGIN_TEMP"
            _red "Run with --clean to remove stale cache, then rerun."
            exit 1
        fi
        git -C "$PLUGIN_TEMP" pull --ff-only || {
            _red "Error: git update failed for module cache at $PLUGIN_TEMP"
            exit 1
        }
    fi
else
